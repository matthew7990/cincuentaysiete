  ...
  return (
    <section id="faq" className="mx-auto max-w-3xl px-4 py-16 md:py-24">
      <h2 className="font-display text-4xl md:text-5xl uppercase text-center">
        Preguntas <span className="text-tomate">frecuentes</span>
      </h2>

      <div className="mt-10 divide-y divide-crema/10 border-y border-crema/10">
        {FAQS.map((f, i) => (
          <div key={f.q}>
            <button
            ...
            >
              ...
              <ChevronDown
                size={18}
                className={`shrink-0 text-queso transition-transform ${open === i ? 'rotate-180' : ''}`}
              />
            </button>
            {open === i && (
              <p className="pb-5 text-sm leading-relaxed text-crema-dim">{f.a}</p>
            )}
          </div>
        ))}
      </div>
